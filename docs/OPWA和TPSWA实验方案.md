# OPWA 与 TPSWA 框架详细设计方案

---

## 一、设计总纲：从 GPPI 实验结论出发的设计原则
...
## 二、OPWA：Orthogonal Plug-in Weather Adapter
...
#### 2.2.2 关键组件一：退化感知分支（Degradation-Aware Branch）

**设计理由**：GPPI 中分支输入是 Albedo（需要额外预计算且仅适用于 rain→sunny），但极端天气场景下我们需要一个**通用的、自适应的分支输入**。根据原则 2（结构化输入有害），不应直接将深度图、透射率图等强结构物理量作为分支输入；根据原则 3（频谱敏感性），分支应接收频域特性与退化相关的信号。
...
**关键区别**：GPPI 的 MultiScaleEncoder 接收的是与内容无关的噪声或 Albedo，而 OPWA 的退化编码器直接接收退化图像。这看似违反了原则 2（结构化输入有害），但我们需要通过**梯度隔离**来规避这一问题。
...
> **设计备选 A'**：如果 stop-gradient 导致分支信息利用不足，可以改为**梯度缩放**（如乘以 0.1），允许少量梯度流回分支 encoder，在"信息利用"和"梯度冲突"之间取得平衡。

#### 2.2.3 关键组件二：条件自适应 Gate（Weather-Conditional Gate）
...
#### 2.2.4 关键组件三：感知驱动损失（Perception-Driven Loss）
...
其中 $\phi_{\text{trunk}}^{(l)}$ 和 $\phi_{\text{branch}}^{(l)}$ 分别是主干 skip 特征和分支注入特征在空间维度展平后的矩阵，$\text{corr}(\cdot,\cdot)$ 是通道间互相关矩阵。这一损失鼓励两个分支的特征在**通道维度**上去相关，而不是在 CKA 的核空间意义上完全独立，计算开销也远小于 CKA。

**权重设置**：$\lambda_o$ 取较小值（如 0.01–0.05），仅作轻度引导，避免过度碎片化。

### 2.3 OPWA 的三个变体方案

考虑到 6 周时间窗口的不确定性，提供三个递进的变体方案：
...
$$
\mathcal{L}_{\text{rec}} = \underbrace{\mathcal{L}_{\text{LF}}(I_{\text{rec}}, I_{\text{clean}})}_{\text{低频→主干主导}} + \underbrace{\mathcal{L}_{\text{HF}}(I_{\text{rec}}, I_{\text{clean}})}_{\text{高频→分支主导}}
$$

其中 $\mathal{L}_{\text{LF}}$ 和 $\mathcal{L}_{\text{HF}}$ 分别是对图像进行高斯低通和高通滤波后计算的 L2 损失。通过梯度流，低频损失主要影响主干（因为主干 skip 连接承载大部分低频信息），高频损失主要影响分支（因为分支通过 gate 注入高频校正）。
...
| 数据源 | 类型 | 用途 |
|--------|------|------|
| WeatherSynthetic | Paired (rain→sunny) | 已有，直接复用 |
...
## 三、TPSWA：Temporal-Physical State-Space Weather Adapter
...
#### 3.2.3 关键组件二：物理一致性损失（Physical Consistency Loss）

**设计理由**：TPSWA 的时序模块需要学习到"什么是雨/雪"的物理特性，而不仅仅是统计上的帧间差异。通过物理一致性损失，引导双流 SSM 的分工符合雨雪的物理规律。
...
其中 $D_t = \text{Decode}(h_t^{\text{deg}})$ 是退化流输出，$\nabla_x$ 和 $\nabla_y$ 分别是水平和垂直梯度。这一损失鼓励退化流输出在垂直方向上的梯度大于水平方向（即雨丝沿垂直方向延伸）。
...
其中 $B_t = \text{Decode}(h_t^{\text{bg}})$ 是背景流输出，$\text{Flow}_{t-1 \to t}$ 是帧间光流（可用预训练光流网络估计）。这一损失约束背景流输出在时间上的一致性。

#### 3.2.4 关键组件三：场景自适应启停（Scene-Adaptive Activation）

**设计理由**：TPSWA 仅在雨/雪场景中有效，在大雾/夜间场景中不仅无益还可能有害（引入时序伪影）。需要一个轻量场景分类器决定何时启用 TPSWA。
...
**核心思路**：不使用完整的 SSM，而是用简单的 3D 卷积（ temporal kernel size = 3）替代，在 3 帧窗口上做时空卷积。
...
| 周次 | OPWA | TPSWA |
|:----:|------|-------|
| W1 | 实现 A1（最小可行版），在 WeatherSynthetic + Foggy Cityscapes 上跑通 | 调研 SSM 实现（Mamba 库），准备 CARLA 数据 |
...
在 3 天实验结束时，用以下框架整理结果，决定 A2 的设计细节：
...
共 4 个对比实验，每个实验在 3 个 test set 上评测：
...
### 6.1 为什么 OPWA 要在 GPPI 基础上做这三项改动？
...
### 6.2 为什么 TPSWA 采用双流 SSM 而非单流 Transformer？

**理由 1：物理可解释性**

双流设计直接对应"背景"和"退化"两种物理实体，每个流的行为可以被独立分析和验证。单流 Transformer 将所有信息混合处理，无法区分模型是在"恢复背景"还是"去除雨丝"。

**理由 2：线性复杂度**

SSM 的时间复杂度为 $O(T)$，而 Transformer 为 $O(T^2)$。在自动驾驶场景中，即使 $T=5$，SSM 的延迟优势也很明显，且可以自然地以在线模式逐帧处理。

**理由 3：可控的记忆长度**

通过初始化 SSM 的 $\mathbf{A}$ 矩阵，可以精确控制每个流的记忆长度：背景流需要长记忆（跨多帧积累信息），退化流需要短记忆（只关注当前帧附近的雨丝动态）。这种细粒度的控制在 Transformer 中难以实现。

### 6.3 为什么物理先验采用软约束而非硬输入？
